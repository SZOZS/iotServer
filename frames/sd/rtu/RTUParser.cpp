...
bool RTUParser::parseReportDataFrame(const std::vector<uint8_t>& frame, size_t& pos, FrameRTUReportData& result) {
  ...
  // 模拟量
  uint16_t* mValues[] = {&result.mValue.m1Value,  &result.mValue.m2Value,  &result.mValue.m3Value,  &result.mValue.m4Value,  &result.mValue.m5Value,  &result.mValue.m6Value,
                         &result.mValue.m7Value,  &result.mValue.m8Value,  &result.mValue.m9Value,  &result.mValue.m10Value, &result.mValue.m11Value, &result.mValue.m12Value,
                         &result.mValue.m13Value, &result.mValue.m14Value, &result.mValue.m15Value, &result.mValue.m16Value};
...
const std::vector<uint8_t>& RTUParser::getFrameHeader() const {
  static const std::vector<uint8_t> header = []() {
    ...
    if(!config) LOG_ERROR("获取SD_RTU协议配置失败");
    return config->header;
  }();
  return header;
}

uint8_t RTUParser::bcdToDecimal(uint8_t bcd) { return ((bcd >> 4) * 10) + (bcd & 0x0F); }
