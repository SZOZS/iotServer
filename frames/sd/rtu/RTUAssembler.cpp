...
bool RTUAssembler::processByte(uint8_t byte) {
  if (isFrameComplete_) reset();  // 处理完一帧后重置，准备下一帧

  // 1. 寻找帧头
  if (!isHeaderFound_) {
    if (byte != FRAME_SD_RTU_START) return false;  // 未找到帧头，忽略当前字节
    isHeaderFound_ = true;
  ...
  if (isDataReceived_ && !isChecksumReceived_) {
    ...
    // LOG_DEBUG_FMT("开始计算校验和，参与字节范围：索引1 至 索引%zu（共%zu字节）", currentBufferSize - 2, (currentBufferSize - 1) - 1); // 计算参与的总字节数
    // （因为当前字节是校验和，在缓冲区中的index为currentBufferSize - 1）
