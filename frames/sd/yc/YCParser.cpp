...
bool YCParser::parse(const std::vector<uint8_t>& frame, std::unique_ptr<FrameBase>& outFrame, std::string& errorMsg) {
  try {
    ...
    for (int i = 0; i < 6; ++i) {
      ...
      LOG_DEBUG_FMT("时间标签第%d字节(%s): 0x%02X", i + 1, (i == 0 ? "秒" : i == 1 ? "分" : i == 2 ? "时" : i == 3 ? "日" : i == 4 ? "月" : "年"), baseFrame->timeTag[i]);
    }
    ...
    LOG_DEBUG_FMT("源地址 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", baseFrame->sourceAddress[0], baseFrame->sourceAddress[1], baseFrame->sourceAddress[2],
                  baseFrame->sourceAddress[3], baseFrame->sourceAddress[4], baseFrame->sourceAddress[5]);
    ...
    if (calculatedChecksum != sdFrame.checksum) {
      LOG_DEBUG_FMT("校验和不匹配，计算值: 0x%02X (%hhu), 实际值: 0x%02X (%hhu)", calculatedChecksum, calculatedChecksum, sdFrame.checksum, sdFrame.checksum);
      errorMsg = "校验和不匹配";
      return false;
    } else {
      LOG_DEBUG("校验和通过");
...
bool YCParser::parseSendData01Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData01& result) {
  ...
  if (result.objCount > 102) LOG_ERROR("信息对象数目错误,大于102");
  ...
  for (uint8_t i = 0; i < result.objCount; ++i) {
    ...
    LOG_DEBUG_FMT("状态发生时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", infoObj.statuOccurrenceTime[0], infoObj.statuOccurrenceTime[1], infoObj.statuOccurrenceTime[2],
                  infoObj.statuOccurrenceTime[3], infoObj.statuOccurrenceTime[4], infoObj.statuOccurrenceTime[5]);
...
bool YCParser::parseSendData02Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData02& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 22) LOG_ERROR("信息对象数目错误,大于22");
  ...
  for (uint8_t i = 0; i < result.objCount; ++i) {
    ...
    infoObj.componentAddress = (static_cast<uint32_t>(frame[pos])) | (static_cast<uint32_t>(frame[pos + 1]) << 8) | (static_cast<uint32_t>(frame[pos + 2]) << 16) |
...
bool YCParser::parseSendData03Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData03& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 63) LOG_ERROR("信息对象数目错误,大于63");
  ...
  for (uint8_t i = 0; i < result.objCount; ++i) {
    ...
    LOG_DEBUG_FMT("模拟量值采样时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", infoObj.analogValueSampleTime[0], infoObj.analogValueSampleTime[1],
                  infoObj.analogValueSampleTime[2], infoObj.analogValueSampleTime[3], infoObj.analogValueSampleTime[4], infoObj.analogValueSampleTime[5]);
...
bool YCParser::parseSendData04Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData04& result) {
  ...
  for (uint8_t i = 0; i < result.objCount; ++i) {
    ...
    LOG_DEBUG_FMT("操作记录时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", infoObj.operationRecordTime[0], infoObj.operationRecordTime[1], infoObj.operationRecordTime[2],
                  infoObj.operationRecordTime[3], infoObj.operationRecordTime[4], infoObj.operationRecordTime[5]);
...
bool YCParser::parseSendData05Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData05& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 1) LOG_ERROR("信息对象数目错误,大于1");
...
bool YCParser::parseSendData06Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData06& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 3) LOG_ERROR("信息对象数目错误,大于3");
...
bool YCParser::parseSendData07Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData07& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 26) LOG_ERROR("信息对象数目错误,大于26");
...
bool YCParser::parseSendData08Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData08& result) {
  ...
  LOG_DEBUG_FMT("建筑消防设施的系统时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", result.buildingFireFacilitySysTime[0], result.buildingFireFacilitySysTime[1],
                result.buildingFireFacilitySysTime[2], result.buildingFireFacilitySysTime[3], result.buildingFireFacilitySysTime[4], result.buildingFireFacilitySysTime[5]);
  return true;
}
...
bool YCParser::parseSendData21Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData21& result) {
  ...
  LOG_DEBUG_FMT("建筑消防设施的系统时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", result.statuOccurrenceTime[0], result.statuOccurrenceTime[1], result.statuOccurrenceTime[2],
                result.statuOccurrenceTime[3], result.statuOccurrenceTime[4], result.statuOccurrenceTime[5]);
  return true;
}
...
bool YCParser::parseSendData24Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData24& result) {
  ...
  result.objCount = frame[pos++];  // 解析信息对象数目
  LOG_DEBUG_FMT("信息对象数目: 0x%02X", result.objCount);
  if (result.objCount > 127) LOG_ERROR("信息对象数目错误,大于127");
...
bool YCParser::parseSendData25Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData25& result) {
  ...
  result.softwareVersion = (static_cast<uint16_t>(frame[pos])) | (static_cast<uint16_t>(frame[pos + 1]) << 8);  // 软件版本号(2字节，具体格式参考协议定义，如主版本号+次版本号)
...
bool YCParser::parseSendData28Frame(const std::vector<uint8_t>& frame, size_t& pos, FrameSendData28& result) {
  ...
  // LOG_DEBUG_FMT("用户信息传输装置系统时间: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", result.userInfoTransmissionDeviceSysTime[0], result.userInfoTransmissionDeviceSysTime[1],
...
uint8_t YCParser::bcdToDecimal(uint8_t bcd) { return ((bcd >> 4) * 10) + (bcd & 0x0F); }
