...
IotSession::IotSession(boost::asio::io_context& io_context, CServer* server) : m_iotSession_socket(io_context), m_iotSession_server(server), m_iotSession_bClose(false) {
  // 生成UUID作为会话唯一标识
...
// 析构函数：输出会话销毁信息
IotSession::~IotSession() { LOG_WARNING_FMT("[%s]已销毁", m_iotSession_UUID.c_str()); }

// 获取会话的socket
boost::asio::ip::tcp::socket& IotSession::getSocket() { return m_iotSession_socket; }

// 获取会话的UUID
std::string& IotSession::getUuid() { return m_iotSession_UUID; }
...
void IotSession::start() {
  ...
  m_iotSession_socket.async_read_some(
    boost::asio::buffer(m_iotSession_data, MAX_LENGTH),
...
void IotSession::send(std::string msg, int msgid) {
  ...
  boost::asio::async_write(m_iotSession_socket, boost::asio::buffer(msgnode->m_msgNodeSend_data, msgnode->m_msgNodeSend_totalLen),
...
void IotSession::close() {
  ...
  if (m_iotSession_bClose) return;  // 避免重复关闭
...
std::shared_ptr<IotSession> IotSession::sharedSelf() { return shared_from_this(); }
  ...
  try {
    ...
    if (!m_currentProtocol.empty() && m_frameAssembler) {
      ...
      for (uint8_t byte : std::vector<uint8_t>(m_iotSession_data, m_iotSession_data + bytes_transferred)) {
        // LOG_DEBUG_FMT("[%s]处理字节: 0x%02X", m_iotSession_UUID.c_str(), static_cast<uint32_t>(byte));  // 打印当前处理的字节（十六进制格式，补零显示两位）
