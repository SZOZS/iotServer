...
IotSession::IotSession(boost::asio::io_context& io_context, CServer* server, short port, const std::string& mainProtocol)
    : m_iotSession_socket(io_context), m_iotSession_server(server), m_iotSession_bClose(false), m_port(port), m_mainProtocol(mainProtocol)
{
    // 生成UUID作为会话唯一标识
    ...
    LOG_INFO_FMT_DELAY("[%s]初始化完成，端口:%d，大协议类型:%s", m_iotSession_UUID.c_str(), m_port, m_mainProtocol.c_str());
}

// 析构函数：输出会话销毁信息
IotSession::~IotSession()
{
    LOG_WARNING_FMT_DELAY("[%s]已销毁", m_iotSession_UUID.c_str());
}
...
{
    LOG_INFO_FMT_DELAY("[%s]启动IotSession，开始初始化CoAP子协议映射...", m_iotSession_UUID.c_str());

    // 从配置类获取「所有CoAP子协议-端口」映射关系
    m_coapSubProtoPortMap = PortProtocolConfig::getInstance().getCoapSubProtoPortMap();

    // 筛选当前端口支持的CoAP子协议列表（核心：缩小识别范围）
    auto port_it = m_coapSubProtoPortMap.find(m_port);
    if (port_it != m_coapSubProtoPortMap.end()) {
        // 直接赋值整个子协议列表（无需循环遍历）
        m_currentPortSubProtos = port_it->second;
    }

    // 打印调试信息，验证子协议列表,不支持则报错退出
    if (m_currentPortSubProtos.empty()) {
        LOG_ERROR_FMT_DELAY("[%s]端口[%d]未配置CoAP子协议，关闭Session", m_iotSession_UUID.c_str(), m_port);
        close();
        m_iotSession_server->clearIotSession(m_iotSession_UUID);
        return;
    } else {
        std::string subProtoStr;
        for (const auto& subProto : m_currentPortSubProtos) {
            subProtoStr += subProto + ",";
        }
        // 移除最后一个逗号（优化日志格式）
        if (!subProtoStr.empty()) {
            subProtoStr.pop_back();
        }
        LOG_INFO_FMT_DELAY("[%s]端口[%d]支持的CoAP子协议：%s", m_iotSession_UUID.c_str(), m_port, subProtoStr.c_str());
    }

    // 启动异步读
    ::memset(m_iotSession_data, 0, MAX_LENGTH);  // 清空接收缓冲区
...
{
    LOG_INFO_FMT_DELAY("[%s]发送数据...", m_iotSession_UUID.c_str());
    // 加锁保护发送队列
    ...
    if (send_que_size > MAX_SEND_QUEUE) {
        LOG_ERROR_FMT_DELAY("[%s]发送失败，超过队列大小[%d]", m_iotSession_UUID.c_str(), MAX_SEND_QUEUE);
        return;
    }

    // 打印待发送数据（转换为十六进制格式，便于查看二进制内容）
    std::string sendHex = utils::UtilsHex::bytesToHexString(msg.c_str(), msg.length());
    LOG_INFO_FMT_DELAY("[%s]待发送数据（msgid: %d, 长度: %zu字节）: %s", m_iotSession_UUID.c_str(), msgid, msg.length(), sendHex.c_str());

    // 将消息封装为SendNode并加入发送队列
    m_iotSession_sendQue.push(std::make_shared<MsgNodeSend>(msg.c_str(), msg.length(), msgid, m_iotSession_UUID));
    LOG_INFO_FMT_DELAY("[%s]消息已排队", m_iotSession_UUID.c_str());
    // 若队列之前为空，立即启动异步发送（否则等待前一条消息发送完成后继续）
    if (send_que_size > 0) {
        LOG_WARNING_FMT_DELAY("[%s]等待发送上一条消息...", m_iotSession_UUID.c_str());
        return;
    }
    // 队列为空时立即发送
...
{
    LOG_WARNING_FMT_DELAY("[%s]开始关闭...", m_iotSession_UUID.c_str());
    ...
    try {
        ...
        LOG_WARNING_FMT_DELAY("[%s]%s:%hu客户端断开连接", m_iotSession_UUID.c_str(), client_ip.c_str(), client_port);
    } catch (const boost::system::system_error& e) {
        LOG_ERROR_FMT_DELAY("[%s]关闭失败，出现以下错误:%s", m_iotSession_UUID.c_str(), e.what());
    }
...
{
    LOG_INFO_FMT_DELAY("[%s]写回调", m_iotSession_UUID.c_str());
    try {
        // 发送成功
        if (!error) {
            std::lock_guard<std::mutex> lock(m_iotSession_sendLock);
            // 移除已发送的消息
            m_iotSession_sendQue.pop();  // 弹出已发送消息
            // 若队列中还有消息，继续发送下一条
            if (!m_iotSession_sendQue.empty()) {  // 队列有剩余，继续发送
                auto& msgnode = m_iotSession_sendQue.front();
                // async_write(..., 写回调);
                boost::asio::async_write(m_iotSession_socket,
                                         boost::asio::buffer(msgnode->m_msgNodeSend_data, msgnode->m_msgNodeSend_totalLen),
            ...
            LOG_ERROR_FMT_DELAY("[%s]写入回调失败，出现以下错误:%s", m_iotSession_UUID.c_str(), error.message().c_str());
            // 关闭会话
            close();
            // 通知服务器移除会话
            m_iotSession_server->clearIotSession(m_iotSession_UUID);
        ...
        LOG_ERROR_FMT_DELAY("[%s]写入回调失败，返回异常代码:%s", m_iotSession_UUID.c_str(), e.what());
    }
}

void IotSession::handleRead(const boost::system::error_code& error, size_t bytes_transferred)
{
    LOG_INFO_FMT_DELAY("[%s]读回调", m_iotSession_UUID.c_str());
    ...
    try {
        ...
        // 缓存接收的字节流 + 打印当前m_recvBuffer的内容（十六进制格式）
        ...
        LOG_INFO_FMT_DELAY("[%s]接收缓冲区更新后内容 (长度: %zu 字节): %s", m_iotSession_UUID.c_str(), m_recvBuffer.size(), recvHex.c_str());

        // 未识别协议时，传入子协议列表限定识别范围
        if (m_currentProtocol.empty()) {
            LOG_INFO_FMT_DELAY("[%s]开始识别CoAP子协议（限定范围：当前端口支持的子协议）", m_iotSession_UUID.c_str());
            // 传入m_currentPortSubProtos，缩小识别范围
            std::string protocol = FrameParserDispatcher::identifyProtocol(m_recvBuffer, m_currentPortSubProtos);
            // 协议识别：根据接收缓冲区的字节流匹配协议（如SD_RTU/HY_LORA）
            // std::string protocol = FrameParserDispatcher::identifyProtocol(m_recvBuffer);
            if (!protocol.empty()) {
                m_currentProtocol = protocol;  // 绑定协议对应的解析器和组装器
                // 工厂模式创建解析器/组装器（面向接口编程，解耦协议实现）
                m_frameParser = FrameParserFactory::createParser(protocol);
                m_frameAssembler = FrameAssemblerFactory::createAssembler(protocol);
                LOG_INFO_FMT_DELAY("[%s]识别到CoAP子协议:%s", m_iotSession_UUID.c_str(), protocol.c_str());
            } else if (m_recvBuffer.size() > MAX_FRAME_LEN) {
                // 超过最大帧长仍未识别，重置缓存（避免内存溢出）
                m_recvBuffer.clear();
                LOG_WARNING_FMT_DELAY("[%s]超过最大帧长未识别到有效CoAP子协议，重置缓存", m_iotSession_UUID.c_str());
            }
        }

        // 已识别协议：逐字节组装帧（解决粘包/拆包）
        if (!m_currentProtocol.empty() && m_frameAssembler) {
            LOG_INFO_FMT_DELAY("[%s]帧解析 开始", m_iotSession_UUID.c_str());
            for (uint8_t byte : std::vector<uint8_t>(m_iotSession_data, m_iotSession_data + bytes_transferred)) {
                // 单帧组装完成
                // LOG_DEBUG_FMT_DELAY("[%s]处理字节: 0x%02X", m_iotSession_UUID.c_str(), static_cast<uint32_t>(byte));  //
                ...
                if (m_frameAssembler->processByte(byte)) {  // 组装完成一帧
                    ...
                    if (FrameParserDispatcher::dispatch(frame, parsedFrame, errorMsg)) {
                        ...
                        LOG_ERROR_FMT_DELAY("[%s]帧解析失败:%s", m_iotSession_UUID.c_str(), errorMsg.c_str());
                    }
        ...
        LOG_ERROR_FMT_DELAY("[%s]读回调异常:%s", m_iotSession_UUID.c_str(), e.what());
        return;
    }
}
...
{
    if (frame) {
        LOG_INFO_FMT_DELAY("[%s]收到解析后的帧，协议类型:%s，消息ID:%d", getUuid().c_str(), frame->protocol.c_str(), frame->msgId);
        if (frame->protocol == "SD_RTU") {
            // SD_RTU协议的帧
            RTUAssembler rtuAssembler;                                                   // 创建RTUAssembler实例，使用其assembleResponse生成响应帧
            std::vector<uint8_t> responseFrame = rtuAssembler.assembleResponse(*frame);  // 生成响应帧
            if (!responseFrame.empty()) {                                                // 仅当响应帧有效时发送
                try {
                    ...
                    if (feedbackMsgId != 0) {
                        send(responseStr, feedbackMsgId);  // 发送反馈
                        LOG_INFO_FMT_DELAY("[%s]反馈帧已发送，长度:%zu字节", getUuid().c_str(), responseFrame.size());
                    } else {
                        LOG_WARNING_FMT_DELAY("[%s]未匹配的SD_RTU消息ID:%d，无法发送反馈", getUuid().c_str(), frame->msgId);
                    }
                } catch (const std::exception& e) {
                    LOG_ERROR_FMT_DELAY("[%s]SD_RTU反馈帧处理失败:%s", getUuid().c_str(), e.what());
                }
            } else {
                LOG_WARNING_FMT_DELAY("[%s]RTUAssembler未生成有效响应帧，消息ID:%d", getUuid().c_str(), frame->msgId);
            }
            ...
            if (!responseFrame.empty()) {  // 仅当响应帧有效时发送
                try {
                    ...
                    LOG_ERROR_FMT_DELAY("[%s]HY_LORA反馈帧处理失败:%s", getUuid().c_str(), e.what());
                }
            } else {
                LOG_WARNING_FMT_DELAY("[%s]LoRaAssembler未生成有效响应帧，消息ID:%d", getUuid().c_str(), frame->msgId);
            }
            ...
            LOG_INFO_FMT_DELAY("[%s]暂不处理协议:%s的反馈", getUuid().c_str(), frame->protocol.c_str());
        }
        ...
        LOG_WARNING_FMT_DELAY("[%s]解析后的帧为空", getUuid().c_str());
    }
}