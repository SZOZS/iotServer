...
{
    LOG_INFO_FMT("[%s]会话销毁", _uuid.c_str());
}

boost::asio::ip::tcp::socket& WebSession::getSocket()
{
    return _socket;
}

std::string& WebSession::getUuid()
{
    return _uuid;
}
...
{
    _socket.async_read_some(boost::asio::buffer(_recv_buf, 1024),
                            std::bind(&WebSession::handleRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
void WebSession::send(const json& data)
{
    ...
    if (_send_que.size() == 1) {
        ...
        boost::asio::async_write(_socket,
                                 boost::asio::buffer(node->m_msgNodeSend_data, node->m_msgNodeSend_totalLen),
...
{
    if (_owner_id != -1)
        WebSessionManager::getInstance().removeOwnerSession(_owner_id, shared_from_this());
    _socket.close();
...
void WebSession::handleRead(const boost::system::error_code& error, size_t bytes_transferred)
{
    ...
    _socket.async_read_some(boost::asio::buffer(_recv_buf, 10240),
                            std::bind(&WebSession::handleRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
void WebSession::handlePhpPush(const json& data)
{
    ...
    for (const auto& session : sessions) {
        if (session && session->isVerified()) {
            session->send(json{
                {"type", "php_push"}, {"code", 200}, {"message", "success"}, {"data", json{{"action_type", "refresh_interface"}, {"interfaces", interfaces}}}});
            sent_count++;
